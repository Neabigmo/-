...
def gradient_spot_check_retry3(obj, y, max_dirs=8, h=8e-7, min_dirs=2):
    ...
    if used < int(min_dirs):
        return dict(
            status="INSUFFICIENT_STABLE_DIRECTIONS",
            error=math.nan,
            active_stable=False,
            **meta,
        )
    return dict(
        status="CHECKED_STABLE_ACTIVE_SET",
        error=float(max(errs)),
        active_stable=True,
        **meta,
    )
...
def _candidate_stream(N, rng, max_trials=64):
    ...
    amps = (
        1e-7, 3e-7, 1e-6, 3e-6, 1e-5, 3e-5,
        1e-4, 3e-4, 1e-3, 2e-3, 3e-3,
        1e-2, 2e-2, 3e-2, 4e-2, 7e-2,
    )
    coords = []
...
def gradient_audit_rows_retry3(W, Ns, target_stable=5, max_trials=64):
    """Adaptive strict preflight; N coverage and thresholds are not relaxed."""
    rows = []
    rng = np.random.default_rng(27112026)

    for N in Ns:
        obj = CoherentObjective(
            N, W, q_low=QLOW, q_high=QHIGH, A=5, ridge=RIDGE, dps=180
        )
        stable_count = 0
        ...
        for trial, (name, y) in enumerate(_candidate_stream(N, rng, max_trials=max_trials)):
            try:
                ...
                rows.append(dict(
                    ...
                    test=name,
                    trial=trial,
                    status="GRADIENT_TEST_EXCEPTION",
                    grad_fd_error=math.nan,
            ...
            gc = gradient_spot_check_retry3(
                obj, y, max_dirs=min(8, len(y)), h=8e-7, min_dirs=2
            )
            rows.append(dict(
                N=N,
                test=name,
                trial=trial,
                status=gc["status"],
                grad_fd_error=gc["error"],
                checked_directions=gc["count"],
                ...
                scientific_valid=bool(ev.scientific_valid),
                eval_status=ev.status,
                prefix_energy=float(ev.prefix_energy),
                tail_lb=float(ev.tail_lb),
                ball_slack=5.0 - float(y @ y),
