...
def gradient_spot_check_retry3(obj, y, max_dirs=8, h=8e-7, min_dirs=2):
    ...
    for j in range(min(len(y), max_dirs)):
        ...
        if ep.status != base.status or em.status != base.status:
            rejected_branch += 1; continue
        if base.qp is not None and (_active_signature(ep.qp) != sig0 or _active_signature(em.qp) != sig0):
            rejected_active += 1; continue
        fd = (ep.value-em.value)/(2*step); an = float(base.grad[j])
...
def _candidate_stream(N, rng, max_trials=64):
    ...
    for j in (0,dim//4,dim//2,(3*dim)//4,dim-1):
        if j not in coords: coords.append(j)
    emitted = 1
    for k, amp in enumerate(amps):
        if emitted >= max_trials: return
        j = coords[k % len(coords)]
        y = np.zeros(dim,float); y[j] = amp*(-1.0 if k%2 else 1.0)
        yield f"coord_{j}_a{amp:.0e}", y; emitted += 1
        if emitted >= max_trials: return
        z = rng.normal(size=dim); z /= max(np.linalg.norm(z),1e-300)
        yield f"random_a{amp:.0e}_{k}", z*amp; emitted += 1


def gradient_audit_rows_retry3(W, Ns, target_stable=5, max_trials=64):
    """Adaptive strict preflight with a HARD active-tail coverage requirement.

    If no stable nontrivial-tail point is found for an N, prefix-only stable
    rows are deliberately downgraded so the unchanged retry2 preflight gate
    rejects that N instead of silently passing on near-zero checks.
    """
    rows = []
        ...
        for trial,(name,y) in enumerate(_candidate_stream(N,rng,max_trials=max_trials)):
            try:
                ...
                rows.append(dict(N=N,test=name,trial=trial,status="GRADIENT_TEST_EXCEPTION",
                    ...
                    rejected_numeric=0,rejected_branch=0,rejected_active=0,scientific_valid=False,
                    eval_status="EXCEPTION",prefix_energy=math.nan,tail_lb=math.nan,active_count=math.nan,
                    tail_regime_signal=False,ball_slack=5.0-float(y@y),qp_success=False,error=repr(exc)))
                continue
            ...
            if not ev.scientific_valid or not qp_success:
                rows.append(dict(N=N,test=name,trial=trial,status="NNQP_REJECTED",
                    grad_fd_error=math.nan,checked_directions=0,attempted_directions=0,
                    rejected_numeric=0,rejected_branch=0,rejected_active=0,scientific_valid=False,
                    eval_status=ev.status,prefix_energy=(float(ev.prefix_energy) if np.isfinite(ev.prefix_energy) else math.nan),
                    tail_lb=tail_lb,active_count=active_count,tail_regime_signal=tail_signal,
                    ball_slack=5.0-float(y@y),qp_success=False,error="base QP not strictly successful"))
                continue

            if stable_count >= target_stable and active_tail_stable >= 1: break
            gc = gradient_spot_check_retry3(obj,y,max_dirs=min(8,len(y)),h=8e-7,min_dirs=2)
            rows.append(dict(N=N,test=name,trial=trial,status=gc["status"],grad_fd_error=gc["error"],
                checked_directions=gc["count"],attempted_directions=gc.get("attempted",0),
                rejected_numeric=gc.get("rejected_numeric",0),rejected_branch=gc.get("rejected_branch",0),
                rejected_active=gc.get("rejected_active",0),scientific_valid=bool(ev.scientific_valid),
                eval_status=ev.status,prefix_energy=float(ev.prefix_energy),tail_lb=tail_lb,
                active_count=active_count,tail_regime_signal=tail_signal,ball_slack=5.0-float(y@y),
                qp_success=qp_success,error=gc.get("exception","")))
            if gc["status"] == "CHECKED_STABLE_ACTIVE_SET":
                stable_count += 1
                if tail_signal: active_tail_stable += 1

        if active_tail_stable < 1:
            for r in rows[first_row:]:
                if r.get("status") == "CHECKED_STABLE_ACTIVE_SET":
                    r["status"] = "CHECKED_PREFIX_ONLY_NO_ACTIVE_TAIL"
    return rows
