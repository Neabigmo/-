...
项目约定 `P_s psi_n=s^(n/2) psi_n` 时，Mehler kernel 的精确平方范数为

`int M_s(x,y)^2 d gamma(x)
 = (1+s)^(-1/2) exp(s y^2/(1+s))`.

网页正文曾写成 `(1-s^2)^(-1/2)`；后者在 `0<s<1` 上是较松的上界因子，因而
不破坏 `||P_(1/2)h||_2<3` 和常数 `8`，但不应继续作为 exact identity 记录。
本目录审计脚本固定采用正确公式，并核验网页所用的较松界仍然安全。
