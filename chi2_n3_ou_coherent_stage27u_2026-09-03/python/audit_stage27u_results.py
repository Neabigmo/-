...
def main():
    ...
    out=pd.read_csv(need(r/'stage27u_outer_search.csv'));cont=pd.read_csv(need(r/'stage27u_continuum_validation.csv'))
    sup=pd.read_csv(need(r/'stage27u_active_support.csv'));fail=pd.read_csv(need(r/'stage27u_failures.csv')) if (r/'stage27u_failures.csv').stat().st_size>1 else pd.DataFrame()
    ...
    if 'grad_fd_error' in out.columns:
        gg=pd.to_numeric(out.grad_fd_error,errors='coerce').dropna(); assert (gg<5e-3).all() if len(gg) else True
    for N in (32,48,64,80):
        assert (r/'candidate_artifacts'/f'N{N}_best.npz').exists()
        z=np.load(r/'candidate_artifacts'/f'N{N}_best.npz',allow_pickle=False)
        assert int(z['N'])==N and abs(float(z['q_high'])-.10)<1e-15 and abs(float(z['q_low'])-.05)<1e-15
        y=np.asarray(z['y_high'],float); W=np.asarray(z['witnesses'],float); assert np.isfinite(y).all() and float(y@y)<=5.000001
    ...
    assert summary['status'] in ('COHERENT_HIGHQ_OBSTRUCTION_STRONG_NUMERIC_EVIDENCE','COHERENT_SURVIVOR_FOUND','COHERENT_OUTER_MINIMAX_UNRESOLVED')
    assert int(summary['failures'])==len(fail)
    print('STAGE27U_OU_COHERENT_AUDIT_OK')
