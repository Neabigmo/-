...
def weak_form_certificate() -> dict[str, object]:
    ...
    polynomial_residuals = [sp.expand(sp.diff(L**k, L) * T - k * L**(k - 1) * T) for k in range(1, 5)]
    require(all(v == 0 for v in polynomial_residuals), "weak polynomial test failed")
    return {
        ...
        "symbolic_residuals": {"frame": [0] * 6, "weak_polynomial_tests_k1_to_4": polynomial_residuals},
