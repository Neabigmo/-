...
def scaled_completion_probe(y_high, N: int, q_low: float=.05, q_high: float=.10,
                            d: int=3, scale: float=.30, energy_cut: float=30.0):
    """Complete the low-q Fock recurrence without converting coefficients to float b_n.

    This is only an algorithmic safety probe.  If the ordinary b conversion would overflow,
    we can still determine from the scaled coefficients whether the high-q prefix energy is
    already above a harmless cutoff (default 30 > A=5).  Such points are excluded from the
    minimax search as PREFIX_OVERFLOW_BARRIER; they are never used as scientific certificates.
    """
    ...
    bodd={d:prob.eps}
    for n,v in zip(prob.free_odds,x):
        bodd[n]=prob.eps*float(v)
    try:
        r,maxres=prob.sys.complete(prob.sys.scaled_r_from_bodd(bodd))
    except Exception as exc:
        return dict(status='SCALED_RECURRENCE_NONFINITE', definitely_over_cut=False,
                    energy_lower_bound=math.nan, max_log_abs_u_high=math.inf,
                    fock_residual=math.nan, error=repr(exc))
    logeps=math.log(prob.eps); ls=math.log(scale); lratio=math.log(float(q_high)/float(q_low))
    threshold=0.5*math.log(float(energy_cut))
    maxlog=-math.inf; partial=0.0
    for n in range(d,N+1):
        z=np.longdouble(r[n])
        if z==0: continue
        la=float(np.log(abs(z))) + 0.5*math.lgamma(n+1) - n*ls - logeps + 0.5*(n-d)*lratio
        maxlog=max(maxlog,la)
        if la > threshold:
            return dict(status='PREFIX_OVERFLOW_BARRIER', definitely_over_cut=True,
                        energy_lower_bound=float(energy_cut), max_log_abs_u_high=float(la),
                        fock_residual=float(maxres), error='')
        partial += math.exp(2.0*la)
        if partial > energy_cut:
            return dict(status='PREFIX_OVERFLOW_BARRIER', definitely_over_cut=True,
                        energy_lower_bound=float(partial), max_log_abs_u_high=float(maxlog),
                        fock_residual=float(maxres), error='')
    return dict(status='SCALED_PROBE_FINITE', definitely_over_cut=False,
                energy_lower_bound=float(partial), max_log_abs_u_high=float(maxlog),
                fock_residual=float(maxres), error='')
