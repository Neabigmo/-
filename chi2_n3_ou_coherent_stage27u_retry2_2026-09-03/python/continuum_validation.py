...
def validate_fixed_candidate(q,N,u,W,ridge=1e-11,dps=220,max_outer=8,
    ...
    for it in range(max_outer):
        ...
        if (lb and li==len(lambda_levels)-1) or (sb and si==len(S_levels)-1):
            if not expanded:
                # W may now include the just-discovered boundary witness. Re-solve
                # on the exact witness set that will be returned/saved so the
                # reported lower bound is reload-consistent.
